...
export const getSmartAccount = async (privateKey: string) => {
  ...
  for (const [i, chain] of supportedEVMNetworks.entries()) {
    const smartAccount = await createLightAccountAlchemyClient({
    ...
    if (i > 0 && smartAccount.account.address != smartAccounts[i - 1].account.address) {
      throw new Error('Smart account addresses are not the same');
    }
  }

  return smartAccounts[0].account.address;
